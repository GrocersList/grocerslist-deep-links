...
export const PostGatingMetaBox = ({ onUpdate }: PostGatingProps) => {
  ...
  const handlePostGatedChange = (event: JSX.TargetedEvent<HTMLInputElement, Event>) => {
  ...
  const handleRecipeCardGatedChange = (event: JSX.TargetedEvent<HTMLInputElement, Event>) => {
    const target = event.target as HTMLInputElement;
    const checked = target.checked;
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Typography variant="body2" sx={{ mt: 1, fontSize: '0.8rem', color: 'text.secondary' }}>
        Recipe Card Gated is a subset of Post Gated. If Post Gated is enabled, the entire post content will be gated.
        If only Recipe Card Gated is enabled, only recipe cards within the post will be gated.
      </Typography>
    </Box>
  );
};
