...
export const DebugMonitorPage = () => {
  ...
  const handleProcessQueue = async () => {
    try {
      ...
      addToast(
        'success',
        `✅ Processed ${result.processed} posts. ${result.errors} errors.`
      );
  ...
  const handleResetFailedPosts = async () => {
    try {
      ...
      addToast(
        'success',
        `✅ Reset ${result.reset} failed posts back to pending`
      );
      ...
      <Stack spacing={3}>
        ...
        <Alert severity="info">
          <Typography variant="body2">
            <strong>How it works:</strong> When you save posts with Amazon
            links, they're added to a queue. A background worker processes them
            every 5 minutes to create linksta URLs and store them in the
            database. This keeps your content rendering fast while ensuring all
            links get processed.
          </Typography>
        </Alert>
          ...
          <CardContent>
            ...
            {queueStats?.nextScheduledRun && (
              <Typography variant="body2" sx={{ mt: 2 }}>
                ...
                {formatDate(queueStats.nextScheduledRun)}
              </Typography>
            )}
          </CardContent>
        </Card>
